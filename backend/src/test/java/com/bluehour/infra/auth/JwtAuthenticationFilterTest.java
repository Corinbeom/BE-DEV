...
class JwtAuthenticationFilterTest {
    ...
    @Test
    @DisplayName("유효한 bluehour_token 쿠키 → SecurityContext에 인증 설정")
    void 유효한_토큰_쿠키() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setCookies(new Cookie("bluehour_token", "valid-token"));
    ...
    void 무효한_토큰_쿠키() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest();
        request.setCookies(new Cookie("bluehour_token", "invalid-token"));
        MockHttpServletResponse response = new MockHttpServletResponse();
